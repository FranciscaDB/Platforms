...
class EncoderReaderNode(Node):
    def __init__(self):
        super().__init__('encoder_reader_left_node')
        #self.get_logger().info("Left encoder started.")

        # Get robot ID from parameter
        self.declare_parameter('ID', 2)
        self.ID = self.get_parameter('ID').get_parameter_value().integer_value

        # Publisher for left wheel linear velocity
        self.speed_L_pub = self.create_publisher(Float32, f'/jetros{self.ID}/speed_L', 2)
        self.last_pulses = 0
